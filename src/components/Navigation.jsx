  ...
  return (
    <nav className='flex flex-col align-middle text-olive font-semibold py-6 sm:py-10 px-6 sm:px-40 w-full top-0 inset-x-0 z-50 fixed backdrop-blur-md sm:h-16'>
      ...
      <div className={`sm:hidden ${isOpen ? 'flex flex-col  self-center' : 'hidden'}`}>
        ...
        <a className='m-auto border-2 rounded-md px-4 py-1 text-pink transition-transform ease-in-out duration-300 hover:scale-110 hover:cursor-pointer' href='/CV_Yesmin_Pizarro_Dev.pdf' download='CV_Yesmin_Pizarro_Dev.pdf'>Resume</a>
      </div>
    </nav>
  )
}
